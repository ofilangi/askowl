...
*/
 ...
 countNodeInservice(service,urinode,option) {
   let req =`PREFIX rdf:<http://www.w3.org/1999/02/22-rdf-syntax-ns#>
...
select distinct count(?elt) as ?c {
  ...
  $.sparqlrequest(conf,req,function(data) {
    ...
    let numberOfNodeAvailInService = results[0].c.value ;
...
/* ===============================================================================================*/
  create() {
    ...
    let listRS = new AskomicsUserAbstraction().getServices();
    let curService = new AskomicsUserAbstraction().getCurrentService();

    let selectRemoteService = $('<select>').attr("urinode",node.uri).attr("uri",node.uri);

    selectRemoteService.append($("<option/>").attr("value",new AskomicsUserAbstraction().endpoint).text("User endpoint"));

    for (let opt in listRS ) {
      let option = $("<option/>").attr("value",listRS[opt]).text(opt);
